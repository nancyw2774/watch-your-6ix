...
async def main():
    ...
    while True:
        ...
        if target_data is None:
            ...
            if is_hazard(target_data, url, hazard, test_mode):
                no_detection_counter = 0
                ...
                if danger_level > 0:
                    if not camera_on:
                        camera_on = True
                        try_request(url, "trigger_event", "4")
                    continue
        no_detection_counter += 1
        # no hazard condition, turn off lights and camera
        if no_detection_counter > 5 and camera_on:
...
def is_hazard(target_data: Target, url, hazard, test = False):
    if hazard == False:
        return False
